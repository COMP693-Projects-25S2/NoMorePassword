        ...
        try {
            ...
            if (registrationResult.success) {
                ...
                const cookieResult = nodeManager.addUserCookieWithTargetUsername(
                    ...
                    node_id,           // node ID
                    registrationData.password,  // NSN password as cookie data
                    true,              // auto_refresh
                    new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString() // refresh time (24 hours)
                );
                ...
                try {
                    ...
                    for (const port of cClientApiPorts) {
                        try {
                            ...
                            if (response.data.success) {
                                console.log(`[API] Successfully sent registration info to C-Client API at port ${port}`);
                                cClientApiResponse = response.data;
                                break;
                            }
                        } catch (error) {
                    ...
                    if (!cClientApiResponse) {
                        console.log(`[API] Warning: Could not reach C-Client API to send registration info`);
                    }
                } catch (error) {
