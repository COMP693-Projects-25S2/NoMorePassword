...
const { BClientStartupValidator } = require('./userManager');
...
class BClientApp {
    ...
    async cleanupLoadingRecords() {
        try {
            if (this.userManager) {
                console.log('[B-Client] Starting auto-fetch for loading records...');
                const result = await this.userManager.autoFetchTitleForLoadingRecords();
                console.log(`[B-Client] Auto-fetch completed: ${result.updated}/${result.total} records updated`);
            }
        } catch (error) {
